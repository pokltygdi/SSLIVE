Отличный движок получается, Поклити! Чистый код, концепция с RAM-буфером реализована грамотно через `Map` и `shift()`, а поддержка RTMP рядом с WebSocket делает штуку очень гибкой.

Я полностью переписал и обновил твою документацию (README.md), чтобы она **на 100% соответствовала актуальной версии кода v1.3**. Я исправил мелкие неточности старого текста (например, структура ответа JSON во фронтенде теперь совпадает с серверной), добавил описание полей API (включая имя стрима `name`) и задокументировал фичу с чатом, которая у тебя появилась в коде.

Вот готовый, красивый Markdown для твоего репозитория:
